...
<section class="reg-import">
    <h1 class="heading-title"> Historial de un reciclaje </h1>
    <div>
        <form action="../../func/tools/updateSolicitud.php?id_imp_exp=<?php echo $_GET['id_imp_exp']?>" id='editUsuario' class='input-group-editUsuario' method="POST">
            ...
            <div class="content">
                ...
                <input name="FechaRH" type="date" min="<?= $hoy ?>" class='input-field' value="<?php if($fecha_recicladora==null){echo '';}else{echo $fecha_recicladora;} ?>" placeholder="Ingrese fecha requerida" readonly required autocomplete="off">
                
            </div>
            
            <div class="form-group">
                <label for="Estado"><b>Estado del retiro</b></label>
                <select class="form-control" name="Estado" id="Estado">
                    <?php
                    if ($estado == "Retiro Pendiente") { ?>
                        <option value="Retiro Pendiente" selected>
                            Retiro Pendiente
                        </option>
                        <option value="Retiro Completado">
                        Retiro Completado
                        </option>
                    <?php } elseif ($estado == "Retiro Completado") { ?>
                        <option value="Retiro Pendiente">
                            Retiro Pendiente
                        </option>
                        <option value="Retiro Completado" selected>
                            Retiro Completado
                        </option>
                    <?php } ?>
                </select>
            ...
            <div>
                <input class="submit-btn" type="submit" value="Cambiar estado" name="Historial">
            </div>
        </form>
    </div>
    </div>
