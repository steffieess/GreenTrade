...
<section class="reg-import">
    ...
    <div>
        <form action="../../func/tools/aceptar.php?id_imp_exp=<?php echo $_GET['id_imp_exp']?>" id='editUsuario' class='input-group-editUsuario' method="POST">
        <h1>Solicitud</h1> <br>
            <?php
            ...
            <br> <h1>Retiro</h1> <br>
            <div class="content">
                <label for="FechaRe"><b>Fecha del retiro</b></label>
                <input name="FechaRe" type="date" min="<?= $fecha_requeridaD ?>" max="<?= $fecha_requeridaH ?>" class='input-field'  placeholder="Ingrese fecha de retiro" required autocomplete="off">     
            </div>

            <div>
                <input class="submit-btn" type="submit" value="Aprobar" name="Aprobar">
            </div>
        </form>
    </div>
    </div>
