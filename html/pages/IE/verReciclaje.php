...
<section class="reg-import">
    <h1 class="heading-title"> Historial del reciclaje </h1>
    <div>
        <form action="../../func/tools/updateSolicitud.php?id_imp_exp=<?php echo $_GET['id_imp_exp']?>" id='editUsuario' class='input-group-editUsuario' method="POST">
            <?php
            ...
            if (mysqli_num_rows($dataSoli) == 1) {
                ...
                $fecha_recicladora = $row['fecha_recicladora'];
                $estado = $row['estado'];

                if($row['empresa_id_empresa'] != NULL){
                    $empresa_id_empresa = $row['empresa_id_empresa'];

                    $sqlReci = ("SELECT * FROM empresa WHERE id_empresa  = '$empresa_id_empresa'");
                    $dataReci = mysqli_query($connc, $sqlReci);
    
                    if (mysqli_num_rows($dataReci) == 1) {
                        $row = mysqli_fetch_array($dataReci);
            ...
            <div class="content">
                <label for="FechaRH"><b>Fecha retiro</b></label>
                <input name="FechaRH" type="text" min="<?= $hoy ?>" class='input-field' value="<?php if($fecha_recicladora==null){echo 'Sin especificar';}else{echo date("d-m-Y", strtotime($fecha_recicladora));} ?>" placeholder="Ingrese fecha requerida" readonly required autocomplete="off">
            </div>

            <div class="content">
                <label for="FechaRH"><b>Recicladora</b></label>
                <input name="FechaRH" type="text" class='input-field' value="<?php if(!isset($razon_social)){echo 'Sin especificar';}else{echo $razon_social;} ?>" placeholder="Ingrese fecha requerida" readonly required autocomplete="off">
            </div>

            <div class="content">
                <label for="FechaRH"><b>Estado</b></label>
                <input name="FechaRH" type="text" class='input-field' value="<?php if($estado==null){echo '';}else{echo $estado;} ?>" placeholder="Estado" readonly required autocomplete="off">
            </div>
