...
<section class="reg-import">
    <h1 class="heading-title"> Solicitar Reciclaje </h1>
    <div>
        <form action="../../func/tools/solicitud.php?id_imp_exp=<?php echo $_GET['id_imp_exp']?>" id='editUsuario' class='input-group-editUsuario' method="POST">
            <?php
            $sqlEmpresa = ("SELECT * FROM empresa WHERE tipo_empresa_id_tipoempresa = 8");
            $dataEmpresa = mysqli_query($connc, $sqlEmpresa);
            ?>
            <div class="form-group">
                <label for="id_empExt"><b>Empresa Recicladora</b></label>
            ...
            <div class="content">
                ...
                <input name="FechaRD" type="date" min="<?= $hoy ?>" class='input-field' value="<?php if($fecha_requeridaD==null){echo '';}else{echo $fecha_requeridaD;} ?>" placeholder="Ingrese fecha requerida" required autocomplete="off">

            </div>

            <div class="content">
                <label for="FechaRH"><b>Fecha requerida hasta</b></label>
                <input name="FechaRH" type="date" min="<?= $hoy ?>" class='input-field' value="<?php if($fecha_requeridaH==null){echo '';}else{echo $fecha_requeridaH;} ?>" placeholder="Ingrese fecha requerida" required autocomplete="off">
                
            </div>

            <div>
                <input class="submit-btn" type="submit" value="Solicitar" name="Solicitar">
            </div>
        </form>
    </div>
    </div>
