...
if (isset($_POST['regUsuarioExt'])) {
    ...
    $queryEmpresa = "SELECT tipo_empresa_id_tipoempresa FROM empresa WHERE id_empresa =  '$id_empExt'";
    $queryTipoEmpresa = mysqli_query($connc, $queryEmpresa);
    while ($dataDE = mysqli_fetch_array($queryTipoEmpresa)) {
        $tipo_empresa = $dataDE['tipo_empresa_id_tipoempresa'];
    }
                ...
                if (!preg_match("/^[0-9.]+[-]?+[0-9kK]{1}/", $rut_user)) {
                    ...
                    if ($dvr == strtoupper($dv)) {
                        ...
                        if ($sqlcantidad <= 0) {
                            ...
                            $queryClave = str_replace(substr($rut_user, -2, 2), '', $rut_user);
                            $password = password_hash($queryClave, PASSWORD_BCRYPT);
                            $queryInsertUsuario = "INSERT INTO usuario(rut_usuario, nom_usuario, ap_paterno, ap_materno, password, mail_usuario, tel_usuario, status, tipo_usu_id_tipousu, empresa_id_empresa) VALUES  ('$rut_user','$nom_user', '$appat_user', '$apmat_user', '$password', '$mail_user','$tel_user', 0, '$tipo_empresa', '$id_empExt')";
